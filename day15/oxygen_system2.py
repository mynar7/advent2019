...
class MazeBot:
  ...
  def probe(self):
    global origin, paths, deadends, greatest_path_length, steps_to_reach_tank, finding_oxygen_paths
    if tank_found and not finding_oxygen_paths:
      steps_to_reach_tank = len(paths) - 1
      origin = make_point_string(position)
      paths = set()
      deadends = set()
      self.probe_state = None
      finding_oxygen_paths = True
    ...
    if self.probe_state == 'PROBING_SURROUNDINGS':
      ...
      # explored all directions
        ...
        # three walls means deadend
          ...
          if len(paths) > greatest_path_length:
            greatest_path_length = len(paths)
...
print(f"steps to tank: {steps_to_reach_tank}")
print(f"tank location: {tank_location}")
print(f"greatest distance to dead end from tank {greatest_path_length - 1}")